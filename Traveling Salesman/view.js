export class View {
    ...
    drawlines() {
        for (let i  = 0; i < this.model.divs.length; i++) {
            if (this.model.divs[i].value !== null) {
                this.con.strokeStyle = "black";
                let city1 = this.model.citiesA[this.model.divs[i].dataset.row];
                let city2 = this.model.citiesA[this.model.divs[i].dataset.col];

                for (let j = 0; j < this.model.lines.length; j++) {
                    if (
                                (this.model.lines[j][1] == city1 && this.model.lines[j][0] == city2)
                            || 
                                (this.model.lines[j][1] == city2 && this.model.lines[j][0] == city1)
                        ) {
    ...
    drawCirles(on) {
        ...
        for (let i = 0; i < this.model.cities; i++) {
            this.con.fillStyle = "black";
            for (let j = 0; j < this.model.citiesTo.length; j++) {
                if (this.model.citiesTo[j] == this.model.citiesA[i]) {
            ...
            this.con.beginPath();
            this.con.arc(this.model.citiesA[i][0], this.model.citiesA[i][1], 15, 0, 2 * Math.PI);
            this.con.fill();
            this.con.fillStyle = "white";
            this.con.fillText(String.fromCharCode("A".charCodeAt(0) + i), this.model.citiesA[i][0], this.model.citiesA[i][1]);
        }
    }
}